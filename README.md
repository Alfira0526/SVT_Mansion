...
| **공유** | 결과 카드 **PNG 저장** · **🐦 트위터 · 💬 카카오톡 · 📸 인스타 스토리** 플랫폼별 공유 (해시태그 + **랜덤 이벤트 문구** 자동 포함, 인스타는 9:16 스토리 이미지) · 원탭 공유(Web Share) + 데스크톱 폴백(트위터 작성창 / 문구 복사) |
| **결과 카드** | 아파트 메타포(옥상 사인·층 태그·창문형 유닛·1F 출입구) · 날짜·해시태그 브랜딩 자동 포함 |
| **🏠 나의 이웃** | 핵심 콘셉트 "**세븐틴이 내 이웃이라면**" — 나(캐럿)를 배치하면 **내 옆집·윗집·아랫집이 누구인지**를 사진·성향과 함께 결과 최상단에 노출. 배치 중에도 실시간으로 이웃이 만들어지는 과정을 보여주고, 공유 문구도 "내 이웃은 도겸·승관·…!"로 자동 구성 |
| **이벤트 엔진** | 멤버 성격 이모지 배지 · **조합/옆집/방향성(좌우·상하)/위치 기반 서사 이벤트**(유닛·부석순·믹스유닛 듀엣·나무위키 케미·고잉세븐틴 포맷·레전드, 총 118개) → "🎬 우리 아파트 이야기" 자동 생성. 데이터는 **[`data/events.js`](data/events.js)로 분리해 관리자가 편집 가능** ([편집 가이드](data/README.md) · [엔진 설계](docs/EVENT_ENGINE.md)) |
...
├─ data/
│  ├─ events.js            # 이벤트/케미 데이터 (관리자 편집)
│  └─ README.md            # 이벤트 편집 가이드
├─ vendor/
