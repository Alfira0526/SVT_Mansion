...
## 권장 방향: 로컬 우선(vendor) + CDN 폴백
`vendor/html2canvas.min.js`(버전 1.4.1, 약 40KB)를 리포에 포함하고, **로컬을 먼저 로드**하되 실패 시 CDN으로 폴백합니다. 완전 오프라인 동작 + 버전 고정 + (원하면) SRI 무결성 검증이 가능합니다.
...
## 대안: 현행 유지 (이중 CDN)
추가 파일 없이 온라인 전제로 충분하다면, 지금의 jsdelivr→unpkg 폴백을 그대로 두어도 됩니다. 별도 조치 불필요.

## 선택 요청
- **(A) 로컬 벤더링** — 오프라인/버전 고정이 필요하면 파일을 확보해 위 절차 적용.
- **(B) 현행 유지** — 온라인 전제로 충분하면 그대로.

두 방향 중 선호를 알려주시면 반영하겠습니다. 파일(약 40KB)만 주시면 (A)를 바로 마무리합니다.
